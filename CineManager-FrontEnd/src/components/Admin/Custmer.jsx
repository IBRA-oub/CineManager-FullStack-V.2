...
export default function Custmer({ allUsers }) {
    ...
    const handletoggleBanStatus = async (userId) => {
                        ...
                        <table className="w-full min-w-max table-auto text-left ">
                            <thead className='bg-[#e5e5e5] '>
                                <tr>
                                    ...
                                    <th className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4">
                                        <p className="block antialiased font-sans text-sm text-black  leading-none font-semibold">ban</p>
                                    </th>
                                            ...
                                            <div className="w-max">
                                                <div className="relative grid items-center font-sans font-bold uppercase whitespace-nowrap select-none bg-green-500/20 text-green-900 py-1 px-2 text-xs rounded-md" >
                                                    <span className={user.banned ? "text-red-500" : "text-green-500"}>
                                                        {user.banned ? "User is banned" : "User is not banned"}
                                                    </span>                                           </div>
                                            </div>
                                        </td>
                                        <td className=" border-b border-blue-gray-50">

                                            <button onClick={() => confirmDeleteUser(user._id)} className="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-900 hover:bg-gray-900/10 active:bg-gray-900/20" type="button">
                                            ...
                                            <button
                                            id="handletoggleBanStatus"
                                                onClick={() => handletoggleBanStatus(user._id)}
                                                className="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-900 hover:bg-gray-900/10 active:bg-gray-900/20" type="button">
                                                <span className="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
                ...
                <div className="fixed inset-0  flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        <div className="text-center p-5 flex-auto justify-center">
                            ...
                            <p className="text-sm text-gray-500 px-8">Do you really want to delete this user?
                        ...
                        <div className="p-3  mt-2 text-center space-x-4 md:block">
                            ...
                            <button onClick={handleDeleteUser} className="mb-2 md:mb-0 bg-red-500 border border-red-500 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg hover:bg-red-600">Delete</button>
                        </div>
                    </div>
                </div>
            )}
