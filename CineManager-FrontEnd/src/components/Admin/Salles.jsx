...
export default function Salles() {
    ...
    const salleDataFunction = (async () => {

        const salleData = await getAllSalle();

        setAllSalle(salleData)
    })
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        const salleData = await creatSalle(data);
        if (salleData) {
            setAllSalle(prevSalle => [...prevSalle, salleData]);
        }
    ...
    // =======================Update===============
    const UpdateSalleSubmit = (SalleId) => {
    ...
    const handleUpdate = async (e) => {
        ...
        const salleData = await updateSalle(selectedSalle, data);
        if (salleData) {
            setAllSalle(prevSalle => [...prevSalle, salleData]);
        }
    ...
    const confirmDeleteSalle = (SalleId) => {
        setSelectedSalle(SalleId);
    ...
    const handleDeleteSalle = async () => {
        const salleData = await deleteSalleApi(selectedSalle);
        if (salleData) {
            setAllSalle(prevSalle => [...prevSalle, salleData]);
        }
                        ...
                        <table className="w-full min-w-max table-auto text-left ">
                            <thead className='bg-[#e5e5e5] '>
                                <tr>
                                    ...
                                    <th className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4">
                                        <p className="block antialiased font-sans text-sm text-black  leading-none font-semibold">place</p>
                                    </th>
                                    <th className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4">
                                        <p className="block antialiased font-sans text-sm text-black  leading-none font-semibold">Type</p>
                                    </th>
                                    ...
                                    <tr key={salle._id}>
                                        <td className="p-4 border-b border-blue-gray-50">
                                            <div className="flex items-center gap-3">
                                                <img src="https://data.pixiz.com/output/user/frame/preview/400x400/6/4/0/5/1495046_f354b.jpg" alt="Spotify" className="inline-block relative object-center w-12 h-12 rounded-lg border border-blue-gray-50 bg-blue-gray-50/50 object-contain p-1" />
                                                <p className="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-bold">{salle.nom}</p>
                                            </div>
                                        </td>
                                        <td className="p-4 border-b border-blue-gray-50">
                                            <p className="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-normal">{salle.places} Seat</p>
                                        </td>
                                        <td className="p-4 border-b border-blue-gray-50">
                                            <p className="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-normal">{salle.type} </p>
                                        </td>


                                        <td className=" border-b border-blue-gray-50">
                                            <button onClick={() => UpdateSalleSubmit(salle._id)} className="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-900 hover:bg-gray-900/10 active:bg-gray-900/20" type="button">
                                            ...
                                            <button onClick={() => confirmDeleteSalle(salle._id)} className="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-900 hover:bg-gray-900/10 active:bg-gray-900/20" type="button">
                                                <span className="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
                ...
                <div className='fixed top-0 left-0 w-full min-h-screen bg-[#00000069]'>
                    <div className='w-full h-screen flex items-center justify-center'>
                        <div className='w-1/3 bg-white p-6 rounded-md relative'>
                            ...
                            <h2 className='text-center font-bold mb-4'>Update Salle</h2>
                            <form onSubmit={handleUpdate} >
                                ...
                                <div className='mb-4'>
                                    ...
                                    <input
                                        type='number'
                                        name='title'
                                        onChange={(e) => setPlaceField(e.target.value)}
                                        className='w-full border border-gray-300 rounded-md p-2'
                                    />
                                </div>
                                <div className='mb-4'>
                ...
                <div className='fixed top-0 left-0 w-full min-h-screen bg-[#00000069]'>
                    <div className='w-full h-screen flex items-center justify-center'>
                        <div className='w-1/3 bg-white p-6 rounded-md relative'>
                            ...
                            <h2 className='text-center font-bold mb-4'>Add New Salle</h2>
                ...
                <div className="fixed inset-0  flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        <div className="text-center p-5 flex-auto justify-center">
                            ...
                            <p className="text-sm text-gray-500 px-8">Do you really want to delete this Salle?
                        ...
                        <div className="p-3  mt-2 text-center space-x-4 md:block">
                            ...
                            <button onClick={handleDeleteSalle} className="mb-2 md:mb-0 bg-red-500 border border-red-500 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg hover:bg-red-600">Delete</button>
                        </div>
                    </div>
                </div>
            )}
